...
struct ScenarioHostData {
  std::vector<std::uint32_t> seeds;
  std::vector<float> source_energy;
  std::vector<float> source_cosine;
  ...
  std::vector<std::uint32_t> collision_count;

  ScenarioHostData(std::size_t scenario_id, std::size_t work_items,
                   std::size_t layers)
      : seeds(work_items), source_energy(work_items),
        source_cosine(work_items), absorption(layers), scattering(layers),
        anisotropy(layers), deposited(work_items, 0.0f),
        reflected(work_items, 0.0f), transmitted(work_items, 0.0f),
        residual(work_items, 0.0f), path_length(work_items, 0.0f),
        collision_count(work_items, 0) {
    auto &seed = seeds;
    auto &energy = source_energy;
    auto &cosine = source_cosine;
    auto &mu_a = absorption;
    auto &mu_s = scattering;
    auto &g = anisotropy;
    auto &dep = deposited;
    auto &refl = reflected;
    auto &trans = transmitted;
    auto &remain = residual;
    auto &path = path_length;
    auto &collisions = collision_count;
...
struct ScenarioBuffers {
  ...
  std::size_t layers;
  ScenarioHostData host;
  celerity::buffer<std::uint32_t, 1> seeds;
  celerity::buffer<float, 1> source_energy;
  celerity::buffer<float, 1> source_cosine;
  celerity::buffer<float, 1> absorption;
  celerity::buffer<float, 1> scattering;
  celerity::buffer<float, 1> anisotropy;
  celerity::buffer<float, 1> deposited;
  celerity::buffer<float, 1> reflected;
  celerity::buffer<float, 1> transmitted;
  celerity::buffer<float, 1> residual;
  celerity::buffer<float, 1> path_length;
  celerity::buffer<std::uint32_t, 1> collision_count;

  ScenarioBuffers(std::size_t id, std::size_t item_count,
                  std::size_t layer_count)
      : scenario_id(id), work_items(item_count), layers(layer_count),
        host(id, item_count, layer_count),
        seeds(host.seeds.data(), celerity::range<1>(item_count)),
        source_energy(host.source_energy.data(),
                      celerity::range<1>(item_count)),
        source_cosine(host.source_cosine.data(),
                      celerity::range<1>(item_count)),
        absorption(host.absorption.data(), celerity::range<1>(layer_count)),
        scattering(host.scattering.data(), celerity::range<1>(layer_count)),
        anisotropy(host.anisotropy.data(), celerity::range<1>(layer_count)),
        deposited(host.deposited.data(), celerity::range<1>(item_count)),
        reflected(host.reflected.data(), celerity::range<1>(item_count)),
        transmitted(host.transmitted.data(), celerity::range<1>(item_count)),
        residual(host.residual.data(), celerity::range<1>(item_count)),
        path_length(host.path_length.data(), celerity::range<1>(item_count)),
        collision_count(host.collision_count.data(),
                        celerity::range<1>(item_count)) {}
};
...
void submit_scenario(celerity::distr_queue &queue, ScenarioBuffers &scenario,
  ...
  queue.submit([&](celerity::handler &cgh) {
    celerity::accessor seeds{scenario.seeds, cgh,
                            celerity::access::one_to_one{},
                            celerity::read_only};
    celerity::accessor source_energy{scenario.source_energy, cgh,
                                    celerity::access::one_to_one{},
                                    celerity::read_only};
    celerity::accessor source_cosine{scenario.source_cosine, cgh,
                                    celerity::access::one_to_one{},
                                    celerity::read_only};
    celerity::accessor absorption{scenario.absorption, cgh,
                                 celerity::access::all{},
                                 celerity::read_only};
    celerity::accessor scattering{scenario.scattering, cgh,
                                 celerity::access::all{},
                                 celerity::read_only};
    celerity::accessor anisotropy{scenario.anisotropy, cgh,
                                 celerity::access::all{},
                                 celerity::read_only};
    celerity::accessor deposited{scenario.deposited, cgh,
                                celerity::access::one_to_one{},
                                celerity::read_write};
    celerity::accessor reflected{scenario.reflected, cgh,
                                celerity::access::one_to_one{},
                                celerity::read_write};
    celerity::accessor transmitted{scenario.transmitted, cgh,
                                  celerity::access::one_to_one{},
                                  celerity::read_write};
    celerity::accessor residual{scenario.residual, cgh,
                               celerity::access::one_to_one{},
                               celerity::read_write};
    celerity::accessor path_length{scenario.path_length, cgh,
                                  celerity::access::one_to_one{},
                                  celerity::read_write};
    celerity::accessor collision_count{scenario.collision_count, cgh,
                                      celerity::access::one_to_one{},
                                      celerity::read_write};

    cgh.parallel_for<McPhotonTransportKernel>(
        celerity::range<1>(work_items), [=](celerity::item<1> index) {
...
Result read_result(celerity::distr_queue &queue,
  ...
  for (auto &scenario_ptr : scenarios) {
    ...
    auto energy_future = queue.fence(scenario.source_energy);
    auto dep_future = queue.fence(scenario.deposited);
    auto refl_future = queue.fence(scenario.reflected);
    auto trans_future = queue.fence(scenario.transmitted);
    auto remain_future = queue.fence(scenario.residual);
    auto path_future = queue.fence(scenario.path_length);
    auto collisions_future = queue.fence(scenario.collision_count);
    auto energy = energy_future.get();
    auto dep = dep_future.get();
    auto refl = refl_future.get();
    auto trans = trans_future.get();
    auto remain = remain_future.get();
    auto path = path_future.get();
    auto collisions = collisions_future.get();
...
int run_calibration(const Config &config) {
  try {
    ...
    auto dep = queue.fence(scenario.deposited).get();
...
int main(int argc, char **argv) {
  ...
  try {
    ...
    const Result result = read_result(queue, scenarios, config);
