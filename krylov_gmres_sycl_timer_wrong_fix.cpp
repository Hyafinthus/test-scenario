...
//
// The default is deliberately safe for a single GPU and for the modified
// scheduler.  A larger scheduler-safe starting point is
// --nx 8192 --ny 8192 --m 4.  Native-only runs may exceed the scheduler's
// signed-int buffer limit with --allow-unsafe-large-buffer 1.
...
// The modified runtime serializes one wait window into a fixed 8192-byte
// message. Twenty-four kernels leaves room for the timed kernels' accessor
// records (at most four each, plus six for the final sampler) while still
// exposing a non-trivial DAG to the scheduler.
constexpr std::size_t kDefaultWindowKernels = 24;
constexpr std::size_t kMaxSafeWindowKernels = 24;
constexpr std::size_t kSchedulerBufferByteLimit =
...
struct Config {
  ...
  std::size_t window_kernels = kDefaultWindowKernels;
  Mode mode = Mode::Full;
  bool verify = false;
  ...
  bool allow_unsafe_large_buffer = false;
};
...
struct MemoryEstimate {
  ...
  bool scheduler_compatible = true;
};
...
struct Buffers {
  ...
  buffer_t &basis_write_guard;
  buffer_t &samples;
};

// This benchmark intentionally uses one queue and one submission thread.  The
// state below retains the last public sycl::event in each wait window, because
// the modified runtime enters scheduleOffline() only through event::wait().
struct SubmissionWindowState {
  std::size_t limit = kDefaultWindowKernels;
  std::size_t pending = 0;
  bool has_pending = false;
  sycl::event last_event;
};

SubmissionWindowState submission_window;
...
void print_usage(const char *program) {
  std::cout
      ...
      << "  --window-kernels <int>         Kernels per scheduler wait window (1..24)\n"
      ...
      << "  --wait-each-kernel <0|1>      Debug-only per-kernel event wait (default 0)\n"
      ...
      << "  --allow-unsafe-large-buffer <0|1>  Bypass the scheduler's 2 GiB buffer guard\n";
}
...
bool parse_arguments(int argc, char **argv, Config &config,
  ...
  for (int arg = 1; arg < argc; ++arg) {
    ...
    } else if (option == "--window-kernels") {
      parsed = parse_positive_size(value, config.window_kernels);
    } else if (option == "--mode") {
    ...
    } else if (option == "--allow-unsafe-large-buffer") {
      parsed = parse_binary_flag(value, config.allow_unsafe_large_buffer);
    } else {
...
bool derive_dimensions_and_memory(Config &config, Dimensions &dimensions,
  ...
  if (config.window_kernels == 0 ||
      config.window_kernels > kMaxSafeWindowKernels) {
    error = "window-kernels must be in [1, 24] for the runtime's 8192-byte message limit";
    return false;
  }
  ...
  if (!checked_multiply(dimensions.h_elements, sizeof(data_t), h_bytes) ||
      !checked_multiply(config.m, sizeof(data_t), coefficient_bytes) ||
      !checked_multiply(config.partials, sizeof(data_t), partial_bytes)) {
    error = "small-buffer byte estimate overflows size_t";
    return false;
  }
  const std::size_t sample_bytes = std::size_t{4} * sizeof(data_t);
  memory.max_buffer_bytes = std::max(
      std::max(memory.vector_bytes, memory.basis_bytes),
      std::max(sample_bytes,
               std::max(h_bytes,
                        std::max(coefficient_bytes, partial_bytes))));
  memory.scheduler_compatible =
      memory.max_buffer_bytes <= kSchedulerBufferByteLimit;
  if (!memory.scheduler_compatible && !config.allow_unsafe_large_buffer) {
    error = "a buffer exceeds the scheduler's signed-int/2-GiB metadata limit; reduce nx/ny/m or pass --allow-unsafe-large-buffer 1 for a native-only run";
    return false;
  }
...
data_t initial_q0_scale(const Config &config,
  ...
  return static_cast<data_t>(1.0 / std::sqrt(q0_squared_norm));
}
...
void initialize_on_host(Buffers &buffers, const Config &config,
  ...
  sycl::host_accessor basis_guard_acc{buffers.basis_write_guard,
                                      sycl::write_only};

  const data_t q0_scale = initial_q0_scale(config, dimensions);
  ...
  basis_guard_acc[0] = static_cast<data_t>(0);
}
...
void initialize_on_device(sycl::queue &queue, Buffers &buffers,
  ...
  sycl::event init_event = queue.submit([&](sycl::handler &cgh) {
    ...
    auto cycle_token_acc = buffers.cycle_token.get_access<
        sycl::access::mode::discard_write>(cgh);
    auto basis_guard_acc = buffers.basis_write_guard.get_access<
        sycl::access::mode::discard_write>(cgh);
    ...
    cgh.parallel_for<KrylovDeviceInitKernel>(sycl::range<1>(init_items),
      ...
      if (index < dimensions.basis_elements) {
        ...
        const data_t scale =
            basis_index == 0 ? q0_scale : other_basis_scale;
        basis_acc[index] =
            scale * basis_seed(basis_index, local_index, config.ny);
      }
...
void configure_submission_window(std::size_t limit) {
  submission_window.limit = limit;
  submission_window.pending = 0;
  submission_window.has_pending = false;
}

void flush_submission_window() {
  if (!submission_window.has_pending) {
    return;
  }
  // event::wait(), rather than queue::wait(), is required by this runtime to
  // invoke handler::scheduleOffline() for all command groups in the window.
  submission_window.last_event.wait();
  submission_window.pending = 0;
  submission_window.has_pending = false;
}

template <typename CommandGroup>
sycl::event submit_command(sycl::queue &queue, bool wait_each_kernel,
                           CommandGroup &&command_group) {
  sycl::event submitted =
      queue.submit(std::forward<CommandGroup>(command_group));
  submission_window.last_event = submitted;
  submission_window.has_pending = true;
  ++submission_window.pending;

  if (wait_each_kernel ||
      submission_window.pending >= submission_window.limit) {
    flush_submission_window();
  }
  return submitted;
}
...
void submit_stencil(sycl::queue &queue, bool wait_each_kernel,
                    ...
                    bool input_is_flat_basis = false) {
  submit_command(queue, wait_each_kernel, [&](sycl::handler &cgh) {
    auto input_acc = input_is_flat_basis
                         ? input.get_access<sycl::access::mode::read>(cgh)
                         : input.get_access<sycl::access::mode::read>(
                               cgh, sycl::range<1>(n),
                               sycl::id<1>(input_offset));
    ...
    cgh.parallel_for<KrylovStencilApplyKernel>(sycl::range<1>(n),
      ...
      const std::size_t source_index =
          (input_is_flat_basis ? input_offset : 0) + index;
      const std::size_t row = index / ny;
      const std::size_t col = index - row * ny;
      const data_t center = input_acc[source_index];
      const data_t north = row > 0 ? input_acc[source_index - ny]
                                   : static_cast<data_t>(0);
      const data_t south = row + 1 < nx ? input_acc[source_index + ny]
                                         : static_cast<data_t>(0);
      const data_t west = col > 0 ? input_acc[source_index - 1]
                                   : static_cast<data_t>(0);
      const data_t east = col + 1 < ny ? input_acc[source_index + 1]
                                        : static_cast<data_t>(0);
...
void submit_orthogonalization_seed(sycl::queue &queue, bool wait_each_kernel,
  ...
  submit_command(queue, wait_each_kernel, [&](sycl::handler &cgh) {
    ...
    auto basis_acc = basis.get_access<sycl::access::mode::read>(cgh);
    ...
    cgh.parallel_for<KrylovIndependentBranchKernel>(
        sycl::range<1>(n), [=](sycl::item<1> item) {
          ...
          auxiliary_acc[index] =
              ...
              static_cast<data_t>(0.003) *
                  (rhs_acc[index] - basis_acc[basis_offset + index]);
        });
  });
}
    ...
    cgh.parallel_for<KrylovDotStage1Kernel>(
        sycl::range<1>(partial_count), [=](sycl::item<1> item) {
          ...
          for (std::size_t index = begin; index < end; ++index) {
            sum += basis_acc[basis_offset + index] * w_acc[index];
          }
...
void submit_dot_stage2(sycl::queue &queue, bool wait_each_kernel,
  ...
  submit_command(queue, wait_each_kernel, [&](sycl::handler &cgh) {
    ...
    // The modified scheduler tracks only the buffer identity, not accessor
    // offsets. Preserve the rest of the small H/scalar buffer explicitly.
    auto scalar_acc =
        scalar_output.get_access<sycl::access::mode::read_write>(cgh);
    cgh.parallel_for<KrylovDotStage2Kernel>(
        sycl::range<1>(1), [=](sycl::item<1>) {
          ...
          scalar_acc[scalar_output_offset] = sum;
        });
  });
}
...
void submit_axpy_update(sycl::queue &queue, bool wait_each_kernel,
  ...
  submit_command(queue, wait_each_kernel, [&](sycl::handler &cgh) {
    ...
    auto basis_acc = basis.get_access<sycl::access::mode::read>(cgh);
    auto h_acc = h.get_access<sycl::access::mode::read>(cgh);
    cgh.parallel_for<KrylovAxpyUpdateKernel>(sycl::range<1>(n),
                                              [=](sycl::item<1> item) {
      const std::size_t index = item[0];
      w_acc[index] -=
          h_acc[h_offset] * basis_acc[basis_offset + index];
    });
  });
}
  ...
  submit_command(queue, wait_each_kernel, [&](sycl::handler &cgh) {
    ...
    cgh.parallel_for<KrylovNormStage2Kernel>(
        sycl::range<1>(1), [=](sycl::item<1>) {
          ...
          scalar_acc[scalar_output_offset] =
              sycl::sqrt(sum > static_cast<data_t>(0)
                             ? sum
                             : static_cast<data_t>(0));
        });
  });
...
void submit_normalize(sycl::queue &queue, bool wait_each_kernel,
                      ...
                      buffer_t &output, std::size_t output_offset,
                      buffer_t &basis_write_guard, std::size_t n) {
  submit_command(queue, wait_each_kernel, [&](sycl::handler &cgh) {
    auto input_acc = input.get_access<sycl::access::mode::read>(
        cgh, sycl::range<1>(n), sycl::id<1>(input_offset));
    auto norm_acc = norm.get_access<sycl::access::mode::read>(cgh);
    // Q is a flat multi-vector, but the scheduler only understands whole
    // buffers. A whole-buffer read_write chain preserves all earlier q_i when
    // q_(k+1) is produced on another device.
    auto output_acc = output.get_access<sycl::access::mode::read_write>(cgh);
    // The one-element write requirement makes this kernel ineligible for the
    // runtime's dim-0 SNMD split. Its global range N does not match Q's (m+1)N
    // memory range, so split merge would otherwise target the wrong Q region.
    auto guard_acc = basis_write_guard.get_access<
        sycl::access::mode::discard_write>(cgh);
    cgh.parallel_for<KrylovNormalizeKernel>(sycl::range<1>(n),
                                             [=](sycl::item<1> item) {
      const std::size_t index = item[0];
      const data_t denominator = norm_acc[norm_offset];
      output_acc[output_offset + index] =
          denominator > kNormFloor ? input_acc[index] / denominator
                                   : static_cast<data_t>(0);
      if (index == 0) {
        guard_acc[0] = denominator;
      }
    });
  });
}
...
void submit_restart_marker(sycl::queue &queue, bool wait_each_kernel,
  ...
  submit_command(queue, wait_each_kernel, [&](sycl::handler &cgh) {
    ...
    auto last_basis_acc = basis.get_access<sycl::access::mode::read>(cgh);
    auto token_acc = cycle_token.get_access<
        sycl::access::mode::discard_write>(cgh);
    cgh.parallel_for<KrylovRestartMarkerKernel>(
        sycl::range<1>(1), [=](sycl::item<1>) {
          const data_t h_value = h_acc[last_norm_offset];
          const data_t q_value = last_basis_acc[last_basis_offset];
...
void submit_final_projection(sycl::queue &queue, bool wait_each_kernel,
  ...
  submit_command(queue, wait_each_kernel, [&](sycl::handler &cgh) {
    ...
    // The scheduler moves Q at whole-buffer granularity; fanout still controls
    // the algorithmic inner loop in x += Q*y.
    auto basis_acc = basis.get_access<sycl::access::mode::read>(cgh);
...
void submit_copy_basis(sycl::queue &queue, bool wait_each_kernel,
                       ...
                       buffer_t &basis_write_guard, std::size_t n) {
  submit_command(queue, wait_each_kernel, [&](sycl::handler &cgh) {
    // One whole-buffer accessor matches the scheduler's buffer-granular
    // coherence model and avoids duplicate accessors being mode-combined.
    auto basis_acc = basis.get_access<sycl::access::mode::read_write>(cgh);
    auto guard_acc = basis_write_guard.get_access<
        sycl::access::mode::discard_write>(cgh);
    cgh.parallel_for<KrylovCopyBasisKernel>(sycl::range<1>(n),
                                             [=](sycl::item<1> item) {
      const std::size_t index = item[0];
      const data_t value = basis_acc[source_offset + index];
      basis_acc[destination_offset + index] = value;
      if (index == 0) {
        guard_acc[0] = value;
      }
    });
  ...
  for (std::size_t k = 0; k < config.m; ++k) {
    ...
    if (use_stencil) {
      ...
      submit_stencil(queue, config.wait_each_kernel, buffers.basis, qk_offset,
                     buffers.w, 0, config.nx, config.ny, dimensions.n, true);
    } else {
    ...
    submit_normalize(queue, config.wait_each_kernel, buffers.w, 0, buffers.h,
                     ...
                     buffers.basis_write_guard, dimensions.n);
  }
}
  ...
  for (std::size_t cycle = 0; cycle < config.cycles; ++cycle) {
    ...
    if (cycle + 1 < config.cycles) {
      ...
      submit_copy_basis(queue, config.wait_each_kernel, buffers.basis,
                        last_basis_offset, 0, buffers.basis_write_guard,
                        dimensions.n);
    }
  }
}
...
void submit_operator_only_mode(sycl::queue &queue, const Config &config,
  ...
  bool last_result_in_w = false;
  // Choose the first ping-pong destination from total-step parity so the last
  // result always lands in w without adding a non-operator copy to this mode.
  const bool first_result_in_w =
      (config.cycles % 2 != 0) && (config.m % 2 != 0);
  for (std::size_t cycle = 0; cycle < config.cycles; ++cycle) {
    for (std::size_t step = 0; step < config.m; ++step) {
      if (first_step) {
        buffer_t &first_output =
            first_result_in_w ? buffers.w : buffers.residual;
        submit_stencil(queue, config.wait_each_kernel, buffers.basis, 0,
                       first_output, 0, config.nx, config.ny, dimensions.n,
                       true);
        first_step = false;
        last_result_in_w = first_result_in_w;
      } else if (last_result_in_w) {
        // Keep ping-pong destinations in N-element buffers. Writing q_0 inside
        // the (m+1)N basis buffer cannot be split correctly by this runtime.
        submit_stencil(queue, config.wait_each_kernel, buffers.w, 0,
                       buffers.residual, 0, config.nx, config.ny,
                       dimensions.n);
        last_result_in_w = false;
      } else {
        submit_stencil(queue, config.wait_each_kernel, buffers.residual, 0,
                       buffers.w, 0, config.nx, config.ny, dimensions.n);
        last_result_in_w = true;
      }
    }
  }
  primary_kind = 2;
...
void submit_samples(sycl::queue &queue, bool wait_each_kernel, Buffers &buffers,
  ...
  submit_command(queue, wait_each_kernel, [&](sycl::handler &cgh) {
    ...
    auto basis_acc =
        buffers.basis.get_access<sycl::access::mode::read>(cgh);
    auto w_acc = buffers.w.get_access<sycl::access::mode::read>(cgh);
    auto auxiliary_acc =
        buffers.auxiliary.get_access<sycl::access::mode::read>(cgh);
    // Sampling is deliberately a small unsplit kernel; moving multi-gigabyte
    // inputs to split four sparse work-items is never worthwhile.
    auto guard_acc = buffers.basis_write_guard.get_access<
        sycl::access::mode::discard_write>(cgh);
    auto sample_acc =
    ...
    cgh.parallel_for<KrylovSampleKernel>(
        sycl::range<1>(4), [=](sycl::item<1> item) {
          ...
          if (primary_kind == 1) {
            primary = basis_acc[basis_offset + index];
...
int main(int argc, char **argv) {
  ...
  std::cout << "CONFIG nx=" << config.nx << " ny=" << config.ny
            ...
            << " window_kernels=" << config.window_kernels
            ...
            << " allow_unsafe_large_buffer="
            << (config.allow_unsafe_large_buffer ? 1 : 0) << '\n';
  std::cout << "MEMORY vector_bytes=" << memory.vector_bytes
            << " Q_bytes=" << memory.basis_bytes
            << " total_estimated_bytes=" << memory.total_bytes
            << " host_backing_bytes=" << memory.total_bytes << '\n';
  std::cout << "RUNTIME max_buffer_bytes=" << memory.max_buffer_bytes
            << " scheduler_buffer_limit=" << kSchedulerBufferByteLimit
            << " compatible=" << (memory.scheduler_compatible ? 1 : 0)
            << '\n';

  try {
    const auto init_begin = std::chrono::steady_clock::now();

    // The distributed runtime's shared-memory path calls getUserPtr() on each
    // buffer. Host-backed construction (as in the local reference examples)
    // guarantees that staging pointer exists and remains alive through all
    // buffer destruction/write-back.
    std::vector<data_t> x_storage(dimensions.n);
    std::vector<data_t> b_storage(dimensions.n);
    std::vector<data_t> basis_storage(dimensions.basis_elements);
    std::vector<data_t> w_storage(dimensions.n);
    std::vector<data_t> residual_storage(dimensions.n);
    std::vector<data_t> auxiliary_storage(dimensions.n);
    std::vector<data_t> partial_storage(config.partials);
    std::vector<data_t> h_storage(dimensions.h_elements);
    std::vector<data_t> coefficient_storage(config.m);
    std::vector<data_t> restart_norm_storage(1);
    std::vector<data_t> cycle_token_storage(1);
    std::vector<data_t> basis_write_guard_storage(1);
    std::vector<data_t> sample_storage(4);

    buffer_t x_buf{x_storage.data(), sycl::range<1>(dimensions.n)};
    buffer_t b_buf{b_storage.data(), sycl::range<1>(dimensions.n)};
    buffer_t basis_buf{basis_storage.data(),
                       sycl::range<1>(dimensions.basis_elements)};
    buffer_t w_buf{w_storage.data(), sycl::range<1>(dimensions.n)};
    buffer_t residual_buf{residual_storage.data(),
                          sycl::range<1>(dimensions.n)};
    buffer_t auxiliary_buf{auxiliary_storage.data(),
                           sycl::range<1>(dimensions.n)};
    buffer_t partial_buf{partial_storage.data(),
                         sycl::range<1>(config.partials)};
    buffer_t h_buf{h_storage.data(),
                   sycl::range<1>(dimensions.h_elements)};
    buffer_t coefficient_buf{coefficient_storage.data(),
                             sycl::range<1>(config.m)};
    buffer_t restart_norm_buf{restart_norm_storage.data(), sycl::range<1>(1)};
    buffer_t cycle_token_buf{cycle_token_storage.data(), sycl::range<1>(1)};
    buffer_t basis_write_guard_buf{basis_write_guard_storage.data(),
                                   sycl::range<1>(1)};
    buffer_t sample_buf{sample_storage.data(), sycl::range<1>(4)};
    Buffers buffers{x_buf,          b_buf,       basis_buf, w_buf,
                    ...
                    basis_write_guard_buf, sample_buf};
    ...
    configure_submission_window(config.wait_each_kernel
                                    ? std::size_t{1}
                                    : config.window_kernels);
